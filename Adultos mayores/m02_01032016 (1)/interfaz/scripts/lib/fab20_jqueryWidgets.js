 ...
 ********************************/
	...
	$('.elementoflotante').each(function(){
		$(this).css({'width': $(this).attr('ancho')+'px', 'height': $(this).attr('alto')+'px', 'left': $(this).attr('izquierda')+'px', 'top': $(this).attr('arriba')+'px', 'background-image': 'url("contenidos/modulo01/media/'+$(this).attr("fondo")+'")'});
		cubierta = '<div class="cubierta"><h3 class="titulo">'+$(this).find('.titulo').text()+'</h3></div>'
		$(this).prepend(cubierta)
		$(this).find('.cubierta').click(function(){
			$(this).slideUp('slow');
		});
	});
	...
	else
	{
			if((escena!=undefined)&&(escena < totalescenas)){
				$('.personaje').each(function(){
					$(this).find('img').attr('src', 'contenidos/modulo01/personajes/' + $(this).attr('imgcalla'));
				});
				if($('#ventana-dialogo div[numero="'+escena+'"]').attr('oculta') == 'todos'){
					$('.personaje').fadeOut(0);
				} else if(($('#ventana-dialogo div[numero="'+escena+'"]').attr('oculta') != '') && ($('#ventana-dialogo div[numero="'+escena+'"]').attr('oculta') != undefined)){
					personajes = $('#ventana-dialogo div[numero="'+escena+'"]').attr('oculta').split(',');
					console.log(personajes);
					for(x=0; x<personajes.length; x++){
						personaje = personajes[x];
						$('.personaje[nombre="'+ personaje +'"]').fadeOut(0);
					}
				}
				if($('#ventana-dialogo div[numero="'+escena+'"]').attr('muestra') == 'todos'){
					$('.personaje').fadeIn(0);				
				}else if(($('#ventana-dialogo div[numero="'+escena+'"]').attr('muestra') != '') && ($('#ventana-dialogo div[numero="'+escena+'"]').attr('muestra') != undefined)){
					personajes = $('#ventana-dialogo div[numero="'+escena+'"]').attr('muestra').split(',');
					console.log(personajes);
					for(x=0; x<personajes.length; x++){
						personaje = personajes[x];
						$('.personaje[nombre="'+ personaje +'"]').fadeIn(0);
					}
				}
				$('.bocadillo').fadeOut(0);
				setTimeout(function(){document.getElementsByTagName('audio')[escena].play()}, 200);
				$('#cargador-imagen-dialogo img[imagen="'+escena+'"]').fadeIn("slow");
				if (($('#ventana-dialogo div[numero="'+escena+'"]').attr('muestra')!= '') && ($('#ventana-dialogo div[numero="'+escena+'"]').attr('muestra')!= 'undefined')){
					$('.personaje[nombre="'+$('#ventana-dialogo div[numero="'+escena+'"]').attr('muestra')+'"]').fadeIn(300);
				}
				if (($('#ventana-dialogo div[numero="'+escena+'"]').attr('habla')!= '') && ($('#ventana-dialogo div[numero="'+escena+'"]').attr('habla')!= 'undefined')){
					dato = $('#ventana-dialogo div[numero="'+escena+'"]');
					$('.bocadillo').html($(dato).html());
					$('.bocadillo').attr('class', 'bocadillo');
					$('.bocadillo').addClass($(dato).attr('flecha'));
					$('.bocadillo').css({'width': $(dato).attr('bwidth')+'px', 'top': $(dato).attr('btop')+'px', 'left': $(dato).attr('bleft')+'px'});
					habla = $('.personaje[nombre="'+$(dato).attr('habla')+'"]');					
					$(habla).find('img').attr('src', 'contenidos/modulo01/personajes/' + $(habla).attr('imghabla'));
...
function escenasplay(){
	...
	if($('#ventana-dialogo').length){
		...
		$('#ventana-dialogo').find('div.personaje').each(function(){
			rutapersonajes = 'contenidos/modulo01/personajes/' + $(this).attr('imgcalla');
			alto = $(this).attr('alto')+'px';
			pleft = $(this).attr('pleft')+'px';
			ptop = $(this).attr('ptop')+'px';
			
			$(this).append('<img src="'+rutapersonajes+'" style="height:'+alto+';">');
			$(this).css({'left': pleft, 'top' : ptop});

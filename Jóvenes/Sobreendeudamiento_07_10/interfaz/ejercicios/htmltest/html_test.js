...
var CLASS_BtnVolver=".btn_volver"
...
function HTML_TEST(c,num)
{
	...
	this.btnVolver		= $($(c).find(CLASS_BtnVolver))
	...
	this.RUTAIMG="interfaz/ejercicios/htmltest/"
	...
	this.srcImgBien=this.RUTAIMG +"bien40px.png"
	...
	this.MSG_sinmarcar="<img src='./contenidos/modulo01/personajes/m_pose1.png' style='position:absolute;'/><div class='contenedorFeedback'><h3 class='dowhat'>"+getTextual("txt_test_sinmarcar1")+"</h3><p>"+getTextual("txt_test_sinmarcar2")+"</p></div>"
	...
	this.reto=this.cuestionario.attr("reto")
	this.activaZona=this.cuestionario.attr("activaZona")
	...
	this.getFeedbackPreguntasMarcadas=function()
	{
		...
		for(cont=0;cont<resp.length;cont++)
		{
			...
			if (($(resp[cont]).find("img").attr("src")==this.srcImgOk)||($(resp[cont]).find("img").attr("src")==this.srcImgMal)) marcada= true
			else   marcada=false
	...
	this.showPregunta=function()
	{
		...
		for(cont=0; cont<this.ejdata.getNumRespuestas();cont++)
		{
			...
			str=str + "<div class='ej_check'><img src='"+this.srcImgKo+"' /></div>"			
	...
	this.showFeedback=function(txt,ico,siguientepregunta)
	{
		...
		//this.mensajes.css("display:block")
			...
			//this.icoPreg	unta="inf_pregunta"
...
//	this.getFeedbackPreguntasMarcadas=function()
		...
		else 
		{
			...
			if(resultado==this.respuestaBIEN)
			{
				...
				// discriminamos segun ejercicio simple o respuesta multiple
					respuesta+= "<img src='./contenidos/modulo01/personajes/feedback_ok.png' style='position:absolute;'/>"
				if (this.seleccion_multiple){
					respuesta+= "<div class='contenedorFeedback'>" + this.MSG_feedMULTIOK;
				} else {
					respuesta+="<div class='contenedorFeedback'><h3 class='ok'>" + this.MSG_feedOK + "</h3><p>"					
				}
				...
				var logroAlPrimerIntento=false
				if((this.reto!=='undefined')&&(this.reto!=='')){
					
					var practica_completa=indiceUnidad.practicaGetValue(this.reto,"completado")
					msg_debug("Ejercicio incorrecto  completado:" + practica_completa )
					if (practica_completa!="true")
					{
						msg_debug("Practica no completa, guardamos puntuacion...")
						var practica_numIntentos=parseInt(indiceUnidad.practicaGetValue(this.reto,"numpaso"))
						practica_numIntentos=practica_numIntentos+1
						//alert(practica_numIntentos)
						var practicaPuntos=50
						if (practica_numIntentos==1) {
							practicaPuntos=100
							logroAlPrimerIntento=true
						}
						
						indiceUnidad.practicaSetValues(this.reto,1,0,true,practica_numIntentos,100,practicaPuntos);
					}
				}
				...
				if (this.tipoFeedback == this.feedRespuesta	)
				{
					...
					//alert(auxfeedresp)
					if (auxfeedresp!="") respuesta+=auxfeedresp;
				}
				if (this.tipoFeedback == this.feedPregunta	)
				{
					...
					//
					var auxfeedresp
					if (logroAlPrimerIntento) auxfeedresp="Has respondido correctamente a la pregunta estrella de este apartado y has conseguido una insignia, enhorabuena." + "</p><p style='text-align:center'><img src='./contenidos/modulo01/media/insigniaConseguida.png' /></p></div>"
					else auxfeedresp="Has respondido correctamente a la pregunta estrella de este apartado, enhorabuena."+ "</p></div>"
					//alert(auxfeedresp)
					if (auxfeedresp!="") respuesta+=auxfeedresp;
				}
				...
				this.btnEnviar.css("display","none");	       // Solo cuando lo haga bien
				this.btnVolver.css("display","inline-block");	
			...
			else
			{
				...
				if (practica_completa!="true")
				{
					...
					practica_numIntentos=practica_numIntentos+1
					//alert(practica_numIntentos)
					var practicaPuntos=0
							
					
					indiceUnidad.practicaSetValues(this.reto,1,0,true,practica_numIntentos,100,0);
				}
		
				this.FALLOS++
				// avisamos de que no es correcta, feedback adecuado, respuesta si esta habilitado
				var  respuesta=""
				respuesta+= "<img src='./contenidos/modulo01/personajes/personaje_error.png' style='position:absolute;'/>"
				
				respuesta+="<div class='contenedorFeedback'><h3 class='ko'>" + this.MSG_feedKO + "</h3><p>"
		...
		if((this.activaZona!=='undefined')&&(this.activaZona!=='')){
			
				msg_debug("Estado practica "+this.reto+ " " + indiceUnidad.practicaGetValue(this.reto,"completado"))
				if (indiceUnidad.practicaGetValue(this.reto,"completado")==true)
				{
						msg_debug("Marcamos la hotzone completa dado que la practica esta completa:" + practica_completa )
					HOTZONE_COMPLETE(this.activaZona);
				}
				else
				{
						msg_debug("NO Marcamos la hotzone completa dado que la practica no esta completa:" + practica_completa )
	...
	this.btnVolver.click(function(){
		//alert("dd")
		indiceUnidad.loadPage('2_3');
	})	
	...
	this.finalizar=function()
	{
			...
			str=str + "<div class='btn_mensajeRepetir btnPrincipal' ><span>"+getTextual("txt_test_repetircuestionario")+"</span><img class='btnFlecha' src='./interfaz/tema/media/btnPrincipalFlecha.png'/></div>"

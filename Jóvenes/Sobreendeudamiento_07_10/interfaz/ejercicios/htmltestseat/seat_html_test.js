...
function F20_SEAT_inicializarCuestionarios()
{
	
	//if ( $('.ej_visorEjercicio').attr('evaluable')=='true') desactivaNavegacion();
	var aux;
	//alert("inicializarCuestionarios")
	$(".ej_visorEjercicioSEAT").each(function(i){
		//alert(" cuestionario " +  i)		
		aux=new F20_SEAT_HTML_TEST(this,i)
		//aux.showPregunta(1)
	})
...
function SCORM_actualizaSeguimiento()
{
		saveTESTVariables()		// actualizamos variables suspendata
		SCORM_cierreSesion_checkFinish(true,false)  // guardamos seguimiento en plataforma
}

function F20_SEAT_HTML_TEST(c,num)
{
	...
	// Numero de intentos
	this.INTENTOS_MAX=indiceUnidad.test_intentostest
	if (SCORM_intentosTEST!="infinitos")
	{
		this.numIntento=SCORM_intentosTEST // El número de intentos deberia de recuperarse del scorm!!!
		this.numIntento--
	}
	JSTHEME_refresh()
	...
	if (SCORM_intentosTEST!="infinitos")
	{	
		SCORM_intentosTEST =this.numIntento
	}
	SCORM_actualizaSeguimiento()
	...
	this.MSG_sinmarcar="<h3 class='dowhat'>"+getTextual("txt_test_sinmarcar1")+"</h3><p>"+getTextual("txt_test_sinmarcar2")+"</p>"   //Debes marcar alguna de las respuestas</h3><p>Prueba de nuevo.
	...
	JSTHEME_refresh()	
	$(".btn_enviar").html(getTextual("txt_enviar"));
	...
	/*
		...
		// Actualmente este parametro lo actualizamos en el xml de definición del curso.
	...
	this.seleccion_multiple=indiceUnidad.test_seleccion_multiple
	//alert(this.seleccion_multiple)
	...
	this.ejdata=new F20_SEAT_CUESTIONARIO()
	...
	{
		//alert("LLamamos a clickoff" + (this.seleccion_multiple) + "  --  " +  (!this.seleccion_multiple)) 
		if(indiceUnidad.test_seleccion_multiple!=true)
		{
			//alert("No esta habilitada la seleccion multiple")
	...
	{
		var cont				
		var strRespuestaCorrecta="<p>"+getTextual("txt_test_strRespuestaCorrecta")+"</p>"		//"<p>La respuesta correcta es:</p>"		
	...
	{
		var strIntentos=""
		/*if ((this.numIntento-1) >0)
		{
			strIntentos="Intentos restantes: " + (this.numIntento -1)
		}
		else
		{
			strIntentos=""
	...
	// MOSTRAMOS EL FEEDBACK SOLO SI HAY ALGUNO DE LOS DOS TIPOS DE FEEDBACK DEFINIDOS
			if ((this.ejdata.getNumPreguntas()>1)&&(((this.tipoFeedback == this.feedPregunta	)||(this.tipoFeedback == this.feedRespuesta	))))
					this.numeroPregunta.html("[ "+getTextual("txt_test_aciertos")+":"+ this.ACIERTOS + " / "+getTextual("txt_test_fallos")+":" + this.FALLOS + 
					" ] &nbsp; " + strIntentos + " &nbsp;" +this.ejdata.getNumPreguntaActual() + "/" + this.ejdata.getNumPreguntas())
			else
					this.numeroPregunta.html(strIntentos+ " &nbsp; "+getTextual("txt_test_pregunta")+":" + this.ejdata.getNumPreguntaActual() + "/" + this.ejdata.getNumPreguntas())
	}
	...
	this.showPregunta=function()
	{
		...
		//alert(n)
		//this.enunciado.html("<b>enunciado " +  this.indiceCuestionario + " .... ruta:  "  + this.xmlsrc + " CFGPregunta: '"  + this.cfgpregunta + "' <br><br>"+ this.ejdata.getEnunciado()+ "<br></b>") 
		...
		//	str= str + "(" + cont + ") " + this.ejdata.getRespuesta(cont) +  " <b>" +  this.ejdata.getRespuestaCorrecta(cont) + "</b><br> " 
									//+  this.ejdata.getRespuestaFeedback(cont)  +"<br>"
	...
	//  ----------------------------------- show mensaje -------------------------------------------------------------------------------------------------------
	this.showFeedback=function(txt,ico,siguientepregunta,tipoventana)
	{
		...
		if ((tipoventana!=null)&&(tipoventana=="ventanafinal"))
		{

			$(".ej_mensaje_ico").css("display","none");
			$(".ej_mensaje").css("width","750px");
			$(".ej_mensaje").css("left","120px");
			$(".ej_mensaje_text").css("padding-left","91px");
			$(".ej_mensaje_text").css("width","85%");
			$(".ej_mensaje_text").css("float","left");		
	...
	this.evaluarResultado=function()
	{
	 if (!this.enableFunctions) exit()
		...
		//this.feedGeneral	= "general"
			...
			//this.icoPregunta="inf_pregunta"
		//	this.icoCorrecto="inf_exito.png"
		//	this.MSG_feedOK="Debes de marcar alguna de las respuestas del enunciado.<br>Prueba de nuevo."
		//this.MSG_feedKO="Debes de marcar alguna de las respuestas del enunciado.<br>Prueba de nuevo."
...
//	this.getFeedbackPreguntasMarcadas=function()
		...
		else 
		{
			...
			{	
				...
				{
						// mostramos botón finalizar SIEMPRE Y CUANDO UTILICEMOS VENTANAS FEEDBACK
						if ((this.tipoFeedback == this.feedPregunta	)||(this.tipoFeedback == this.feedRespuesta	))
						{
			...
			if(resultado==this.respuestaBIEN)
			{
				...
				if (this.seleccion_multiple){
					respuesta=respuesta + this.MSG_feedMULTIOK;
				} else {
				...
				{
					//alert("mostramos feed respuestas")
					var auxfeedresp="<br><br>"+this.ejdata.getPreguntaFeedback()+ "<br><br>"
					//alert(auxfeedresp)
					if (auxfeedresp!="") respuesta=respuesta + ""  + auxfeedresp + ""
				}
			...
			else
			{
				...
				{
					//alert("mostramos feed respuestas")
					var auxfeedresp="<br><br>"+this.ejdata.getPreguntaFeedbackNegativo() + "<br><br>"
					//alert(auxfeedresp)
					if (auxfeedresp!="") respuesta=respuesta + ""  + auxfeedresp + ""
				}
			...
			// SI NO ESTA CONFIGURADO NINGUN TIPO DE FEEDBACK 
			if (!((this.tipoFeedback == this.feedPregunta	)||(this.tipoFeedback == this.feedRespuesta	)))
			{
				...
				if (this.ejdata.esUltimaPregunta())
				{
					// Si hemos respondido a la ultima pregunta finalizamos
					this.finalizar();
				}
				else
				{
					// caso contrario avanzamos a la siguiente pregunta
	...
	this.finalizar=function()
	{
			...
			var resultado=Math.round(this.ACIERTOS/this.ejdata.getNumPreguntas()*100);
			var str=""
			...
			str=str + "<tr><td>"+getTextual("txt_test_puntuacion")+":</td><td> "+ resultado + "% </td></tr>"
			
			if (resultado>=indiceUnidad.test_nota) // Deberia ser nota de corte que es inferior a 100 (mastery score)
			{
				
				SCORM_testSuperado=true
				SCORM_resultadoTEST=resultado
				
				
				str=str + "<tr><td colspan=2> "+getTextual("txt_test_enhorabuena")+"</td></tr></table><br><br>" //Enhorabuena, has superado el TEST.
				
				str=str + "<button id='btn_salircuestionario' class='btn_mensajeRepetir' >"+getTextual("txt_test_salircuestionario")+"</button>"  //Repetir el cuestionario
				if	(
					(indiceUnidad.test_blocktestwhencompleted!=true)&&
					( 
						(SCORM_intentosTEST=="infinitos")
						||
						(SCORM_intentosTEST>0)
					)
					)
				{
					str=str + "<button id='btn_mensajeRepetir' class='btn_mensajeRepetir' >"+getTextual("txt_test_repetircuestionario")+"</button>"  //Repetir el cuestionario
				}
				else
				{
					if ((SCORM_intentosTEST!="infinitos")&&(SCORM_intentosTEST<=0)) str=str + getTextual("txt_test_nointentos") //"Lo sentimos, ya no dispone de más intentos."
			...
			{
				if(SCORM_testSuperado) 
				{
						// Si el test ya estaba superado no lo suspendemos
				}
				else
				{
						SCORM_testSuperado=false
						SCORM_resultadoTEST=resultado
				}
				
				
				str=str + "<tr><td colspan=2> "+getTextual("txt_test_noenhorabuena") //Lo sentimos, no ha superado el test.
				
				//SCORM_estado= getValor("cmi.core.lesson_status")
				if (SCORM_testSuperado==true) 		str=str + getTextual("txt_test_testcompletado")  // mensaje indicando que le test ya se supero anteriormente

				str=str   +"</td></tr></table><br><br>"
				
				//
				//this.numIntento--;
				// AQUI HEMOS DE ALMACENAR EL NUMERO DE INTENTOS EN EL SCORM
			
				if((SCORM_intentosTEST!="infinitos")&&(SCORM_intentosTEST<=0))
				{
					// Hemos agotado todos los intentos. No pintamos el boton y lo  indicamos con un mensaje.
					str=str + getTextual("txt_test_nointentos") //"Lo sentimos, ya no dispone de más intentos."
					str=str + "&nbsp; <button id='btn_salircuestionario' class='btn_mensajeRepetir' >"+getTextual("txt_test_salircuestionario")+"</button>"  //Repetir el cuestionario
				}
				else
				{
					// Aun nos quedan intentos por realizar.
					str=str + "<button id='btn_mensajeRepetir' class='btn_mensajeRepetir' >"+getTextual("txt_test_repetircuestionario")+"</button>"  //Repetir el cuestionario
				}
			...
			SCORM_actualizaSeguimiento() //  Actualizamos seguimiento en este momento.
			
			
			this.showFeedback(str,this.icoExito, false,"ventanafinal")
			var scope=this
			this.mensajes.find("#btn_mensajeRepetir").click(function(){scope.repetir();})
			this.mensajes.find("#btn_salircuestionario").click(function(){close();})
			
			this.mensajes.find(".ej_mensaje_close").css("display","none");
	...
	this.repetir=function()
	{
		...
		// Volvemos a desordenar el test si esta configurado como aleatorio
		if(this.ejdata.CFG_Aleatorio>0) 
		{
			this.ejdata.preguntas=_initPreguntasAleatorias_F20_SEAT_CUESTIONARIO(this.ejdata.preguntas,$(this.ejdata.xmlObj).find('preguntas').children(),this.ejdata.CFG_Aleatorio);
		}
		...
		if (SCORM_intentosTEST!="infinitos")
		{
			SCORM_intentosTEST=SCORM_intentosTEST-1
		}
		JSTHEME_refresh()
		SCORM_actualizaSeguimiento()
	...
	// En caso de que el número de intentos restantes sea menor que 0 no permitimos hacer el test.
	if (SCORM_intentosTEST!="infinitos")
	{
		...
		{	
				this.showFeedback(getTextual("txt_test_nointentosv2"),this.icoInfo, false)
				$(".ej_visorEjercicioSEAT .acciones").css("display","none")
		}
	}
	else
	{
